    ...
    <script>
        ...
        (function() {
            ...
            function setTheme(theme) {
                ...
                if (theme === 'dark') {
                    sunIcons.forEach(icon => { if(icon) icon.style.display = 'block'; });
                    moonIcons.forEach(icon => { if(icon) icon.style.display = 'none'; });
                } else {
                    sunIcons.forEach(icon => { if(icon) icon.style.display = 'none'; });
                    moonIcons.forEach(icon => { if(icon) icon.style.display = 'block'; });
                }
            }
            ...
            // Terapkan tema segera (sebelum DOM load)
            ...
            // Fungsi toggle untuk dipanggil dari tombol
            window.toggleTheme = function() {
                const isDark = document.documentElement.classList.contains('dark');
                setTheme(isDark ? 'light' : 'dark');
            };
        })();
        // ==================== END DARK MODE GLOBAL SYSTEM ====================
    </script>
    ...
    <div class="max-w-7xl mx-auto flex justify-between items-center py-4 px-4 sm:px-6 lg:px-8">
        ...
        <div class="hidden md:flex items-center gap-3">
            ...
            <div class="relative">
                ...
                <div id="notificationDropdown" class="notification-dropdown">
                    ...
                    <div id="notificationList" class="max-h-80 overflow-y-auto">
                        <div class="p-4 text-center text-gray-400 dark:text-gray-500">Memuat...</div>
                    </div>
                    <div class="p-2 border-t dark:border-gray-700">
                        <a href="{{ route('notifications.index') }}" class="text-blue-600 dark:text-blue-400 text-sm block text-center">Lihat Semua</a>
                    </div>
                </div>
            </div>
            ...
            <a href="{{ route('karyawan.profile') }}" class="profile-link">
                @if($profilePhotoUrl)
                    <img src="{{ $profilePhotoUrl }}" alt="Foto Profil" class="h-7 w-7 rounded-full object-cover border border-gray-200 dark:border-gray-700">
                @else
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        // Set icons awal setelah DOM loaded
        const isDark = document.documentElement.classList.contains('dark');
        const sunIcons = document.querySelectorAll('#sun-icon, #mobile-sun-icon');
        const moonIcons = document.querySelectorAll('#moon-icon, #mobile-moon-icon');
            ...
            fetch('/api/notifications')
                ...
                .then(data => {
                    ...
                    if (data.notifications && data.notifications.length > 0) {
                                ...
                                <div class="notification-item ${!n.is_read ? 'unread' : ''}" onclick="markNotificationRead(${n.id}, '${n.link}')">
                                    <div class="flex gap-3">
                                        ...
                                        <div class="flex-1">
                                            ...
                                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">${n.time_ago}</p>
                                        </div>
                                    </div>
                                </div>
                            `;
                        ...
                        container.innerHTML = '<div class="p-4 text-center text-gray-400 dark:text-gray-500">Tidak ada notifikasi</div>';
                    }
                })
