...
<body class="font-display bg-background-light text-text-light">
    <div class="flex min-h-screen">
        <!-- Sidebar - Dipanggil dari template HR sider -->
                ...
                <div class="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-6 sm:mb-8">
                    ...
                    <div
                        ...
                        <div
                            ...
                            <span class="material-icons-outlined text-primary text-sm sm:text-base">people</span>
                        </div>
                        <div class="min-w-0 flex-1">
                            <p class="label-text text-xs sm:text-sm text-text-muted-light truncate">Total Karyawan</p>
                            <p class="value-text text-base sm:text-xl font-bold truncate" id="totalEmployees">0</p>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div class="min-w-0 flex-1">
                            <p class="label-text text-xs sm:text-sm text-text-muted-light truncate">Status Aktif</p>
                            <p class="value-text text-base sm:text-xl font-bold truncate" id="activeEmployees">0</p>
                        </div>
                    </div>
                    ...
                    <div
                        ...
                        <div
                            ...
                            <span
                                class="material-icons-outlined text-purple-500 text-sm sm:text-base">domain</span>
                        </div>
                        <div class="min-w-0 flex-1">
                            <p class="label-text text-xs sm:text-sm text-text-muted-light truncate">Jumlah Divisi</p>
                            <p class="value-text text-base sm:text-xl font-bold truncate" id="totalDivisions">0</p>
                        </div>
                    </div>
                    ...
                    <div
                        class="stat-card bg-card-light rounded-DEFAULT p-3 sm:p-5 flex flex-col items-center justify-center text-center border border-border-light">
                        <div
                            class="icon-container w-10 h-10 sm:w-12 sm:h-12 bg-orange-100 rounded-lg mb-2 flex items-center justify-center">
                            <span class="material-icons-outlined text-orange-500 text-sm sm:text-base">groups</span>
                        </div>
                        <div class="min-w-0 flex-1">
                            <p class="label-text text-xs sm:text-sm text-text-muted-light truncate">Jumlah Tim</p>
                            <p class="value-text text-base sm:text-xl font-bold truncate" id="totalTeams">0</p>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        // Current user role (injected from server)
        ...
        async function apiFetch(endpoint, options = {}) {
            ...
            let base = '/api';
            if (window.userRole === 'general_manager') {
                base = '/api/general-manager';
            } else if (window.userRole === 'hr') {
                // HR-specific endpoints under /api/hr
                if (endpoint.includes('meeting-notes') || endpoint.includes('announcements')) {
                    base = '/api/hr';
                }
            }
            const url = `${base}${endpoint}${endpoint.includes('?') ? '&' : '?'}${cacheBuster}`;
            const defaultHeaders = { 'Accept': 'application/json', 'X-CSRF-TOKEN': window.csrfToken };
            const finalOptions = Object.assign({ credentials: 'include', headers: Object.assign({}, defaultHeaders, options.headers || {}) }, options);
            const response = await fetch(url, finalOptions);
            
            // Check if response is successful
            if (!response.ok) {
                const errorText = await response.text();
                console.error(`API Error [${response.status}] ${url}:`, errorText.slice(0, 500));
    ...
    async function loadHighlightedDates() {
        try {
            ...
            const datesArray = response.dates || response.data || response || [];
            const dates = Array.isArray(datesArray) ? datesArray : [];
    ...
    // --- INISIALISASI UTAMA ---
    // Load employee statistics for HR dashboard
    async function loadEmployeeStats() {
        try {
            // Load total karyawan using apiFetch
            const karyawanResponse = await apiFetch('/hr/karyawan/count');
            const totalEmployees = karyawanResponse.count || 0;
            document.getElementById('totalEmployees').textContent = totalEmployees;

            // Load active employees
            const activeResponse = await apiFetch('/hr/karyawan/active');
            const activeEmployees = activeResponse.count || 0;
            document.getElementById('activeEmployees').textContent = activeEmployees;

            // Load divisi count
            const divisiResponse = await apiFetch('/divisis/list');
            const divisiArray = Array.isArray(divisiResponse) ? divisiResponse : (divisiResponse.data || []);
            document.getElementById('totalDivisions').textContent = divisiArray.length;

            // For now, set teams to 0 or load if you have an endpoint
            ...
            document.getElementById('totalDivisions').textContent = '0';
            document.getElementById('totalTeams').textContent = '0';
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', async () => {
        ...
        // Load meeting and announcement dates for all roles (now including HR)
