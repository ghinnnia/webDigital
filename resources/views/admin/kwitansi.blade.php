                    ...
                    <div class="border-b pb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" id="hargaLayanan" name="harga_layanan_display"
                                    class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-600 cursor-not-allowed"
                                    readonly>
                                <input type="hidden" id="hargaLayananHidden" name="harga_layanan">
                            </div>
                        </div>
                        <div class="mt-3">
                    ...
                    <div class="border-b pb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            <div>
                                ...
                                <input type="text" id="harga" name="harga_display"
                                    class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-600 cursor-not-allowed"
                                    readonly>
                            ...
                            <div>
                                ...
                                <input type="text" id="pajak" name="tax_display"
                                    class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-600 cursor-not-allowed"
                                    readonly>
                                <input type="hidden" id="pajakHidden" name="tax">
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Fee Maintenance (Rp)</label>
                                <input type="text" id="feeMaintenance" name="fee_maintenance_display"
                                    class="w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-lg text-gray-600 cursor-not-allowed"
                                    readonly>
                                <input type="hidden" id="feeMaintenanceHidden" name="fee_maintenance">
                            </div>
                        </div>
                        <div class="mt-3">
    ...
    <div id="deleteKwitansiModal"
        ...
        <div class="bg-white rounded-xl shadow-lg w-full max-w-md">
            <div class="p-6">
                ...
                <div class="space-y-4">
                    <div class="mb-6">
                        <p class="text-gray-700 mb-2 delete-message">Apakah Anda yakin ingin menghapus kwitansi untuk <span id="deleteKwitansiNama" class="font-semibold"></span> dengan nomor invoice <span id="deleteKwitansiNomor" class="font-semibold"></span>?</p>
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <button type="button" class="px-4 py-2 bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors btn-delete-confirm">Hapus</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    `;
                        ...
                        fetch(`/admin/api/invoice-for-kwitansi/${invoiceId}`, {
                            ...
                            .then(data => {
                                ...
                                if (data.success && data.data) {
                                    ...
                                    setElementValue('hargaLayanan', formatToRupiah(subtotal));
                                    setElementValue('hargaLayananHidden', subtotal);
                                    ...
                                    setElementValue('harga', formatToRupiah(subtotal));
                                    setElementValue('pajak', formatToRupiah(tax));
                                    setElementValue('feeMaintenance', formatToRupiah(feeMaintenance));
                                    setElementValue('hargaHidden', subtotal);
                                    setElementValue('pajakHidden', tax);
                                    setElementValue('feeMaintenanceHidden', feeMaintenance);
                    ...
                    if (data.success && data.data && data.data.length > 0) {
                        ...
                        document.querySelectorAll('.edit-kwitansi-btn').forEach(btn => {
                            btn.addEventListener('click', function() {
                                ...
                                // Removed displayKwitansiNo update as element no longer exists
                        ...
                        document.querySelectorAll('.delete-kwitansi-btn').forEach(btn => {
                            btn.addEventListener('click', function() {
                                ...
                                openDeleteKwitansiModal(id, namaPerusahaan, invoiceNo);
                            });
                        });
        ...
        function submitBuatKwitansi() {
            ...
            fetch('/admin/kwitansi', {
                ...
                .then(data => {
                    if (data.success) {
                        showMinimalPopup('Success', 'Kwitansi berhasil dibuat!', 'success');
                        closeBuatKwitansiModal();
                        loadKwitansiData();
                        loadInvoiceOptions();
                    } else {
        ...
        function submitEditKwitansi() {
            ...
            fetch(`/api/kwitansi/${id}`, {
                ...
                .then(data => {
                    if (data.success) {
                        showMinimalPopup('Success', 'Kwitansi berhasil diperbarui!', 'success');
                        closeEditKwitansiModal();
                        loadKwitansiData();
                    } else {
        ...
        function deleteKwitansi(id) {
            ...
            fetch(`/api/kwitansi/${id}`, {
                ...
                .then(data => {
                    if (data.success) {
                        showMinimalPopup('Success', 'Kwitansi berhasil dihapus!', 'success');
                        closeDeleteKwitansiModal();
                        loadKwitansiData();
                        loadInvoiceOptions();
                    } else {
        ...
        function closeBuatKwitansiModal() {
            ...
            document.getElementById('hargaLayananHidden').value = '';
            document.getElementById('hargaHidden').value = '';
            document.getElementById('pajakHidden').value = '';
            document.getElementById('feeMaintenanceHidden').value = '';
            document.getElementById('totalHidden').value = '';
            document.getElementById('subTotalHidden').value = '';
        ...
        function openDeleteKwitansiModal(id, namaPerusahaan, invoiceNo) {
            ...
            // UPDATED: Set detailed message including Invoice No (Like Invoice Modal)
            const namaSpan = document.getElementById('deleteKwitansiNama');
            const nomorSpan = document.getElementById('deleteKwitansiNomor');

            if (message && namaSpan && nomorSpan) {
                namaSpan.textContent = namaPerusahaan || '?';
                nomorSpan.textContent = invoiceNo || '-';
            }
        ...
        function showMinimalPopup(title, message, type = 'success') {
            ...
            const popupIcon = popup.querySelector('.minimal-popup-icon');
